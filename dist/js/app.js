(function() {
    ...
    function trackResult(success) {
      ...
      if (results.length === 2) {
        ...
        if (allOk) {
          ...
          statusEl.textContent = results.some(function(r) {
            return r;
          }) ? "Partial" : "Mock";
  ...
  document.addEventListener("visibilitychange", function() {
    if (document.hidden) {
      ...
      if (window.__clockInterval) clearInterval(window.__clockInterval);
      window.__clockInterval = setInterval(updateClock, 1e3);
      if (window.__onlineInterval) clearInterval(window.__onlineInterval);
      window.__onlineInterval = setInterval(updateOnline, 8e3);
      if (window.__updatedInterval) clearInterval(window.__updatedInterval);
      window.__updatedInterval = setInterval(updateLastUpdated, 3e4);
  ...
  function updateOnline() {
    ...
    try {
      ...
      var base = users.filter(function(u) {
    ...
    searchInput.addEventListener("keydown", function(e) {
      if (e.key === "Enter" && this.value.trim()) {
        ...
        if (route === "users") {
          ...
          if (us) {
            us.value = query;
            us.dispatchEvent(new Event("input"));
          }
        } else if (route === "transactions") {
          var ts = document.getElementById("transactionSearch");
          if (ts) {
            ts.value = query;
          ...
          setTimeout(function() {
            var us2 = document.getElementById("userSearch");
            if (us2) {
              us2.value = query;
              us2.dispatchEvent(new Event("input"));
            }
          }, 100);
        }
        this.value = "";
  ...
  function renderNotifDropdown() {
    ...
    list.innerHTML = notifs.map(function(n) {
      return '<div class="notif-dropdown__item' + (n.unread ? " notif-dropdown__item--unread" : "") + '"><div class="notif-dropdown__item-icon notif-dropdown__item-icon--' + Utils.escapeHtml(n.type) + '"></div><div class="notif-dropdown__item-content"><div class="notif-dropdown__item-text">' + Utils.escapeHtml(n.text) + '</div><div class="notif-dropdown__item-time">' + Utils.escapeHtml(n.time) + "</div></div></div>";
    }).join("");
  ...
  function setupNotifications() {
    ...
    if (markAllBtn) markAllBtn.addEventListener("click", function() {
      ...
      dropdown.querySelectorAll(".notif-dropdown__item--unread").forEach(function(el) {
  ...
  function showKeyboardHelp() {
    ModalSystem.open(
      '<div class="modal__header"><h2 class="modal__title">Keyboard Shortcuts <button class="modal__close" data-modal-close aria-label="Close"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></h2></div><div class="modal__body" style="max-height:400px;overflow-y:auto"><div class="shortcut-group"><div class="shortcut-row"><span class="shortcut-keys"><kbd>Ctrl</kbd>+<kbd>K</kbd></span><span>Command palette</span></div><div class="shortcut-row"><span class="shortcut-keys"><kbd>Ctrl</kbd>+<kbd>S</kbd></span><span>Quick save</span></div><div class="shortcut-row"><span class="shortcut-keys"><kbd>Ctrl</kbd>+<kbd>Z</kbd></span><span>Undo</span></div><div class="shortcut-row"><span class="shortcut-keys"><kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>Z</kbd></span><span>Redo</span></div><div class="shortcut-row"><span class="shortcut-keys"><kbd>Ctrl</kbd>+<kbd>/</kbd></span><span>Keyboard shortcuts</span></div><div class="shortcut-row"><span class="shortcut-keys"><kbd>Escape</kbd></span><span>Close modal / palette</span></div><div class="shortcut-row"><span class="shortcut-keys"><kbd>G</kbd>+<kbd>D</kbd></span><span>Go to Dashboard</span></div><div class="shortcut-row"><span class="shortcut-keys"><kbd>G</kbd>+<kbd>U</kbd></span><span>Go to Users</span></div><div class="shortcut-row"><span class="shortcut-keys"><kbd>G</kbd>+<kbd>T</kbd></span><span>Go to Transactions</span></div><div class="shortcut-row"><span class="shortcut-keys"><kbd>G</kbd>+<kbd>S</kbd></span><span>Go to Settings</span></div></div></div>',
      function() {
      }
    );
  }
  ...
  function resetGotoBufferTimeout() {
    ...
    gotoBufferTimer = setTimeout(function() {
      gotoBuffer = "";
      gotoBufferTimer = null;
    }, 1500);
  }
  function isTyping() {
