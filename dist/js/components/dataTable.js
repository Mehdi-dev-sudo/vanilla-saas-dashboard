const DataTable = /* @__PURE__ */ (function() {
  function create(containerId, config) {
    ...
    function bindEvents() {
      ...
      if (onSelect) {
        ...
        if (selectAll) selectAll.addEventListener("change", function() {
          selected = this.checked ? data.map(function(r, i) {
            return r.id || i;
          }) : [];
          render();
    ...
    function getSort() {
      return sortKey ? { key: sortKey, dir: sortDir } : null;
    }
    render();
    return { update, getSelected, getSort, render };
  }
  return { create };
})();
