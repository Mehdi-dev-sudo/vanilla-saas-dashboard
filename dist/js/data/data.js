const AppStore = (function() {
  ...
  function defaultState() {
    return {
      ...
      transactions: [
        { id: "t1", invoice: "INV-001", customer: "Acme Corp", email: "billing@acmecorp.com", amount: 12e3, status: "completed", date: "2026-03-28", method: "Wire Transfer" },
        ...
        { id: "t3", invoice: "INV-003", customer: "DevShop Inc", email: "ap@devshop.com", amount: 21e3, status: "pending", date: "2026-03-26", method: "Bank Transfer" },
        ...
        { id: "t6", invoice: "INV-006", customer: "HealthTech Ltd", email: "accounts@healthtech.com", amount: 32e3, status: "completed", date: "2026-03-23", method: "Wire Transfer" },
        ...
        { id: "t8", invoice: "INV-008", customer: "Enterprise Inc", email: "treasury@enterprise.com", amount: 54e3, status: "completed", date: "2026-03-21", method: "Wire Transfer" },
        ...
        { id: "t10", invoice: "INV-010", customer: "SaaS IO", email: "ap@saas.io", amount: 15e3, status: "completed", date: "2026-03-19", method: "ACH" },
        { id: "t11", invoice: "INV-011", customer: "EcomStore", email: "finance@ecomstore.com", amount: 28e3, status: "completed", date: "2026-03-18", method: "Credit Card" },
        { id: "t12", invoice: "INV-012", customer: "DataDrive AI", email: "payments@datadrive.ai", amount: 42e3, status: "pending", date: "2026-03-17", method: "Wire Transfer" },
        ...
        { id: "t15", invoice: "INV-015", customer: "GameDev JP", email: "finance@gamedev.jp", amount: 86e3, status: "completed", date: "2026-03-14", method: "Wire Transfer" },
        { id: "t16", invoice: "INV-016", customer: "Consulting ME", email: "ap@consulting.me", amount: 11e3, status: "completed", date: "2026-03-13", method: "ACH" },
        { id: "t17", invoice: "INV-017", customer: "Blockchain IO", email: "payments@blockchain.io", amount: 67e3, status: "pending", date: "2026-03-12", method: "Wire Transfer" },
        { id: "t18", invoice: "INV-018", customer: "RetailPlus", email: "treasury@retailplus.com", amount: 23e3, status: "completed", date: "2026-03-11", method: "Credit Card" },
        { id: "t19", invoice: "INV-019", customer: "Manufacturing CN", email: "finance@manufacturing.cn", amount: 95e3, status: "completed", date: "2026-03-10", method: "Wire Transfer" },
        ...
        { id: "t22", invoice: "INV-022", customer: "Fashion FR", email: "finance@fashion.fr", amount: 51e3, status: "completed", date: "2026-03-07", method: "Wire Transfer" },
        ...
        { id: "t24", invoice: "INV-024", customer: "Travel RO", email: "invoices@travel.ro", amount: 16e3, status: "completed", date: "2026-03-05", method: "PayPal" },
        { id: "t25", invoice: "INV-025", customer: "Automotive JP", email: "billing@automotive.jp", amount: 78e3, status: "completed", date: "2026-03-04", method: "Wire Transfer" }
      ],
      notifications: 5,
      settings: {
  ...
  function getState(key) {
    if (key) {
      var val = state[key];
      if (val === void 0 && (key === "users" || key === "transactions")) return [];
      if (val === void 0 && key === "settings") return {};
      return val;
    }
    return state;
  }
  ...
  function getFilteredTransactions(search, status, sortBy, sortDir, page, perPage, method) {
    ...
    if (search) {
      const s = search.toLowerCase();
      filtered = filtered.filter(
        (t) => t.customer.toLowerCase().includes(s) || t.invoice.toLowerCase().includes(s) || t.email.toLowerCase().includes(s)
      );
    }
  ...
  function getDashboardStats() {
    try {
      ...
      var us = state.users || [];
      var totalRevenue = tx.filter(function(t) {
        return t && t.status === "completed";
      }).reduce(function(s, t) {
        return s + (t.amount || 0);
      }, 0);
      var activeUsers = us.filter(function(u) {
        return u && u.status === "active";
      }).length;
      var subscribers2 = us.filter(function(u) {
        return u && u.plan !== "Free";
      }).length;
      var churned = us.filter(function(u) {
        return u && (u.status === "inactive" || u.status === "suspended");
      }).length;
      var churnRate = us.length > 0 ? churned / us.length * 100 : 0;
      var prevRevenue = Math.max(totalRevenue * (0.85 + Math.sin(Date.now() / 864e5) * 0.1), 1);
      var prevUsers = Math.max(activeUsers * (0.92 + Math.sin(Date.now() / 432e5) * 0.05), 1);
      var prevSubs = Math.max(subscribers2 * (0.96 + Math.sin(Date.now() / 648e5) * 0.04), 1);
      var prevChurn = Math.max(churnRate * (0.9 + Math.sin(Date.now() / 36e6) * 0.15), 0.01);
      return {
        ...
        subscribers: { value: subscribers2, change: Math.round((subscribers2 - prevSubs) / prevSubs * 100 * 10) / 10, isUp: subscribers2 >= prevSubs },
