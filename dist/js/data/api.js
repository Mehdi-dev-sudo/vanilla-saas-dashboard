const ApiClient = /* @__PURE__ */ (function() {
  var cache = {};
  var retryCount = {};
  ...
  function saveCache(url, data, ttlMs) {
    var key = getCacheKey(url);
    SafeStorage.setObject(key, { data, expiry: Date.now() + (ttlMs || 12e4) });
  }
  function clearCache() {
    var keys = [];
  ...
  function fetchWithTimeout(url, options, timeoutMs) {
    return new Promise(function(resolve, reject) {
      ...
      fetch(url, { ...options, signal: controller.signal }).then(function(r) {
        clearTimeout(timer);
        resolve(r);
      }).catch(function(e) {
        clearTimeout(timer);
    ...
    return fetchWithRetry(url).then(function(data) {
      var users = (data.users || []).map(function(u, i) {
        ...
        return {
          ...
          joined: new Date(Date.now() - Math.random() * 365 * 864e5).toISOString().slice(0, 10),
          revenue: Math.round(Math.random() * 5e3),
          apiSource: true
        };
      });
      saveCache(url, users, 3e5);
      return users;
    });
  }
  function fetchTransactions() {
    ...
    return fetchWithRetry(url).then(function(data) {
      ...
      var transactions = (data.products || []).map(function(p, i) {
        return {
          id: "api-t" + p.id,
          invoice: "INV-" + String(2026e3 + parseInt(p.id)).slice(-7),
          customer: p.title,
          email: p.brand ? p.brand.toLowerCase().replace(/\s+/g, "") + "@example.com" : "vendor@example.com",
          amount: Math.round(p.price * (p.stock !== void 0 ? p.stock : 10) * (0.5 + Math.random())),
          status: statuses[i % statuses.length],
          date: new Date(Date.now() - Math.random() * 30 * 864e5).toISOString().slice(0, 10),
          method: methods[i % methods.length],
          apiSource: true
        };
      });
      saveCache(url, transactions, 3e5);
      return transactions;
    });
  }
  function fetchStats() {
