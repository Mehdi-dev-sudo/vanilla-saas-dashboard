const AnalyticsPage = /* @__PURE__ */ (function() {
  ...
  function drawCharts() {
    ...
    const userData = months.map((m, i) => ({ month: m, value: 110 + i * 18 + Math.round(Math.sin(i * 1.3) * 25) }));
    const refundData = months.map((m, i) => ({ month: m, value: Math.round(200 + Math.sin(i * 0.8) * 120 + i * 15) }));
    ChartEngine.drawLineChart("analyticsRevenueChart", revenueData, { height: 280, prefix: "$", divisor: 1e3, suffix: "k" });
  ...
  function reinitCharts() {
    if (document.getElementById("analyticsRevenueChart")) {
      const months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
      ChartEngine.drawLineChart("analyticsRevenueChart", months.map((m, i) => ({ month: m, value: 18e3 + i * 1400 + Math.round(Math.sin(i) * 2e3) })), { height: 280, prefix: "$", divisor: 1e3, suffix: "k" });
    }
