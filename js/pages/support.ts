      ...
      '<div class="card">' +
        '<div class="card__header">' +
          '<div>' +
            ...
            '<span class="card__subtitle">We typically respond within 24 hours</span>' +
          '</div>' +
        '</div>' +
        '<div class="card__body">' +
          '<div class="form-row">' +
            '<div class="form-group">' +
              ...
              '<input type="text" class="form-input" id="contactName" name="name" placeholder="Your name">' +
              '<span class="form-error" id="contactNameError" role="alert" style="display:none"></span>' +
            '</div>' +
            '<div class="form-group">' +
              '<label class="form-label" for="contactEmail">Email</label>' +
              '<input type="email" class="form-input" id="contactEmail" name="email" placeholder="your@email.com">' +
          ...
          '<div class="form-group">' +
            ...
            '<input type="text" class="form-input" id="contactSubject" name="subject" placeholder="How can we help?">' +
            '<span class="form-error" id="contactSubjectError" role="alert" style="display:none"></span>' +
          '</div>' +
          '<div class="form-group">' +
            '<label class="form-label" for="contactMessage">Message</label>' +
            '<textarea class="form-textarea" id="contactMessage" name="message" placeholder="Describe your issue..." rows="4"></textarea>' +
            '<span class="form-error" id="contactMessageError" role="alert" style="display:none"></span>' +
          '</div>' +
          '<div class="form-actions" style="display:flex;gap:var(--space-sm);margin-top:var(--space-md)">' +
            '<button class="btn btn--primary" id="contactSubmitBtn">Send Message</button>' +
            '<button class="btn btn--secondary" id="contactClearBtn">Clear</button>' +
          '</div>' +
        '</div>' +
      '</div>';
  },
...
function renderFaqItem(index, question, answer) {
  ...
  return '<div class="faq-item" data-faq="' + index + '">' +
...
function setupContactForm() {
  ...
  if (submitBtn) submitBtn.addEventListener('click', function () {
    ...
    if (valid) {
      ...
      [name, email, subject, message].forEach(function (el) { if (el) el.value = ''; });
    }
  });
