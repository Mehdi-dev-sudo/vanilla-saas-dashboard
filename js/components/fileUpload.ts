...
const FileUpload = (function () {
  function create(containerId, opts) {
    ...
    function render() {
      c.innerHTML = '<div class="file-upload" id="fuZone">' +
        ...
        '<p>' + label + '</p>' +
        '<input type="file" id="fuInput" accept="' + accept + '" style="display:none">' +
        '</div>';
      ...
      if (zone) {
        ...
        zone.addEventListener('dragover', function (e) { e.preventDefault(); zone.classList.add('file-upload--dragover'); });
        zone.addEventListener('dragleave', function () { zone.classList.remove('file-upload--dragover'); });
