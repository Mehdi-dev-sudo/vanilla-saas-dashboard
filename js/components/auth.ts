...
const AuthManager = (function () {
  ...
  function updateUserDisplay() {
    ...
    var initials = nameParts.map(function (n) { return n[0]; }).join('').slice(0, 2).toUpperCase();

    var userNameEls = document.querySelectorAll('.sidebar__user-name, #sidebarUserName, .header__profile-name, #headerUserName');
    userNameEls.forEach(function (el) { if (el) el.textContent = user.name; });
    ...
    roleEls.forEach(function (el) { if (el) el.textContent = user.role; });
