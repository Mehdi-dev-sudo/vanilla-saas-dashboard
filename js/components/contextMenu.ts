...
const ContextMenuManager = (function () {
  ...
  function show(x, y, row) {
    ...
    if (type === 'user') {
      items = [
        { label: 'Edit User', icon: 'edit', action: function () { if (typeof UsersPage !== 'undefined') { var btns = row.querySelectorAll('.edit-user-btn'); if (btns.length) btns[0].click(); } } },
        { label: 'Copy Email', icon: 'copy', action: function () { var email = row.dataset.email; if (email) { navigator.clipboard.writeText(email).then(function () { ToastSystem.success('Email copied'); }); } } },
        { label: 'Duplicate', icon: 'copy-plus', action: function () { if (id && typeof AppStore !== 'undefined') { var user = AppStore.getUser(id); if (user) { AppStore.addUser({ name: user.name + ' (Copy)', email: 'copy-' + user.email, role: user.role, plan: user.plan, status: 'active', joined: new Date().toISOString().slice(0, 10), revenue: 0 }); HistoryManager.pushSnapshot(); ToastSystem.success('User duplicated'); if (typeof UsersPage !== 'undefined' && UsersPage.refresh) UsersPage.refresh(); } } } },
        { type: 'separator' },
        { label: 'Delete User', icon: 'trash-2', danger: true, action: function () { if (id) { var delBtns = row.querySelectorAll('.delete-user-btn'); if (delBtns.length) delBtns[0].click(); } } }
      ];
    } else if (type === 'transaction') {
      items = [
        { label: 'View Details', icon: 'eye', action: function () { ToastSystem.info('Transaction: ' + (row.dataset.invoice || '')); } },
        { label: 'Copy Invoice', icon: 'copy', action: function () { var inv = row.dataset.invoice; if (inv) { navigator.clipboard.writeText(inv).then(function () { ToastSystem.success('Invoice copied'); }); } } },
        { type: 'separator' },
        { label: 'Export Row', icon: 'download', action: function () { ToastSystem.success('Row exported'); } }
      ];
    } else {
      return;
    }
